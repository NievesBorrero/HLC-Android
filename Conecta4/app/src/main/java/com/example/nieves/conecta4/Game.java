...
public class Game {
	...
	public int indicaCasilla(int i, int j){
		return tablero[i][j];
	}
   ...
   /**
	 * Identificamos esa casilla (i,j) con el jugador.
 	 * @param i
	 * @param j
	 */
	  public void mueveJugador(int i, int j) {
		  tablero[i][j] = JUGADOR;
	  }
  ...
  /*
  * Comprueba si el jugador puede mover ficha.
   */
  public boolean puedeMover(int i, int j) {
      boolean lastMovement = true;
      for (int pos = i+1 ; pos <NFILAS; pos++){
    	  if(estaVacio(pos, j)){
    		  lastMovement = false;
    	  }
      }
      return lastMovement && estaVacio(i, j);
  }
